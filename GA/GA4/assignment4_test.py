...
def can_turn_off_lights(input_file_path, output_file_path):
    with open(input_file_path, 'r') as inputfile:
        problems = inputfile.read().split("***")[1:]
        #print(problems)
        answers = []
        ...
        for problem in problems:
            ...
            #print("n:",n,"m:", m)
            initialState = list(map(int, lines[1].split(',')))
            #print("initial state: ",initialState)
            connection = [list(map(int, line.split(','))) for line in lines[2:]]
            #print("connection: ",connection)

            formula = two_cnf()
            for l in range(1,m+1):
                s = [i+1 for i, s in enumerate(connection) if l in s]
                #print("switches: ",s)
                if len(s)==2:
                    s1, s2 = s
                    if initialState[l-1]==1:
                        formula.add_clause([str(s1), str(s2)])
                        formula.add_clause(['~'+str(s1), '~'+str(s2)])
                    else:
                        formula.add_clause([str(s1), '~'+str(s2)])
                        formula.add_clause(['~'+str(s1), str(s2)])

            answer = two_sat_solver(formula)
...
can_turn_off_lights("C:\\CS325_Algorithm\\GA\\GA4\\samples_subset\\samples_subset\\test_medium_1.txt", "output.txt")
